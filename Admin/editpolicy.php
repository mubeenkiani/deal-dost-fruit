...
$id = $_GET['Id'];
  ...
  if(isset($_POST['submit'])){
    $editor = $_POST['editor'];
    $data = base64_encode($editor);

    $query = "UPDATE `policy` SET `Privacy_Policy`='$data' WHERE Id = '$id'";
  ...
  $sql = "SELECT * FROM `policy` WHERE Id = '$id'";
    ...
    if (mysqli_num_rows($run)>0) {
      while ($row = mysqli_fetch_assoc($run)) {
        $id = $row['Id'];
        $olddata = base64_decode($row['Privacy_Policy']);
        }
      }
?>
<!DOCTYPE html>
    ...
    <div class="card">
      ...
      <div class="card-body">
        <form class="needs-validation" method="POST" enctype="multipart/form-data" novalidate>
          <textarea class="form-control mb-4" rows="10" id="example" name="editor"><?php echo $olddata ?></textarea>
          <div>
